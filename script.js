...
upload2D.addEventListener('change', async ev=>{
  ...
  for(const f of files){
    ...
    catch(e){ dataURL=await new Promise((res,rej)=>{ const r=new FileReader(); r.onload=e=>res(e.target.result); r.readAsDataURL(f); }); }
...
function render3DPlaneAndCapture(entry){
  ...
  setTimeout(()=>{ try{ entry.snapshot=globalRenderer.domElement.toDataURL('image/png'); }catch(e){ entry.snapshot=null; } renderDesignList(); alert(`3D snapshot captured for "${entry.name}"`); },800);
}
...
generatePDFBtn.addEventListener('click', ()=>{
  ...
  // Embed all designs at the bottom
  let currY=finalY+80;
  for(const d of designs){
    const embed=d.snapshot||d.dataURL; if(!embed) continue;
    if(currY+30>pageHeight-40){ doc.addPage(); drawHeader(); drawFooter(); currY=80; }
    doc.setFontSize(10); doc.setTextColor(20,20,20);
    doc.text(`${d.name}`,margin,currY); currY+=14;
    const imageW=260,imageH=180;
    if(currY+imageH>pageHeight-40){ doc.addPage(); drawHeader(); drawFooter(); currY=80; }
    try{ doc.addImage(embed,getImageTypeFromDataURL(embed),margin,currY,imageW,imageH); }catch(e){ console.warn('Embed design failed',e);}
    currY+=imageH+12;
  }
